(function($) {
  ...
  var methods = {
    ...
    init: function(options) { 
      var settings = $.extend($.EditTable.default_options, options);
    ...
    refresh: function() {
      return this.each(function() {
        ...
        $("td,th",$("tr",$this).eq(0)).each(function() {
          ...
          if(data.settings.columns[colname].class) {
            $(this).addClass(data.settings.columns[colname].class);
          }
          
          // Determine the editor to use for this column
          var editor = data.settings.columns[colname].editor = data.settings.columns[colname].editor || 'text';
          var editor_options = data.settings.columns[colname].options = data.settings.columns[colname].options || {};
        ...
        $("tr",$this).slice(1).each(function() {
          ...
          $("td",$row).each(function() {
            ...
            // If this column has a class applied
            if(data.settings.columns[cell.colname].class) {
              $(this).addClass(data.settings.columns[cell.colname].class);
            }
    ...
    rows: function(rows) {
      ...
      if($.type(rows) === 'array' && rows.length) {
        return this.each(function() {
          ...
          // TODO: make thead an option
          var header_row = $("<tr></tr>").appendTo($("<thead></thead>").appendTo($this));
          var headers = [];
  ...
  $.EditTable.helpers = {
    ...
    centerX: function(editor_el, td) {
      ...
      var dh = editor_el.outerHeight() - editor_el.height();
    ...
    centerY: function(editor_el,td) {
      ...
      var dw = editor_el.outerWidth() - editor_el.width();

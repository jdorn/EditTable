(function($) {
  ...
  window.EditTable = Class.extend({
     ...
     * @param row The row number starting from 0 (doesn't include header)
    ...
    /**
     * Gets a single row as a JSON object
     * @param row The row number starting from 0 (doesn't include header)
     * @return A JSON object
     */
    getRowJSON: function(row) {
      var data = {};
      for(var i=0; i<this._column_order.length; i++) {
        data[this._column_order[i]] = this.get(this.getCell(row, this._column_order[i]));
      }
      return data;
    },
    ...
    goDown: function() {
      ...
      var col = this.getColumn(this._current_cell);
      var row = this.getRow(this._current_cell);

      var next = this.getCell(row+1, col);

      if(next && next.length) {
        this.edit(next);
      }
      // If there isn't a next row, add one
      else {
        this.addRow();
        next = this.getCell(row+1, col);
        this.edit(next);
      }
     ...
     * @returns this Makes the method chainable
     */
    addRow: function(num) {
      this._cell_cache = {};
     ...
     * @param i The row to remove (defaults to the last row)
     * @return this Makes the method chainable
     */
  ...
  EditTable.AbstractEditor = Class.extend({
    ...
    _centerX: function(td) {
      var pos = td.position();

      var dh = this.element.outerHeight() - this.element.height();

      // Fill the parent height, keep editor's width, horizontally center
      var height = td.outerHeight() - dh;
    ...
    _centerY: function(td) {
      var pos = td.position();

      var dw = this.element.outerWidth() - this.element.width();

      // Fill the parent width, keep editor's height, vertically center
      var width = td.outerWidth() - dw;
    ...
    _fill: function(td) {
      ...
      this.element.css({
        top: pos.top,
        left: pos.left,
        width: td.outerWidth() - dw,
        height: td.outerHeight() - dh
      });
    },
    /**
     * Show the editor
